...
매일 아침(KST 07:00) RSS · Reddit · Hacker News · GitHub에서 CTI(Cyber Threat Intelligence)
관련 새 글을 수집하고, Claude가 [context.md](context.md)(위협 인텔리전스 구조화·분석 관심사) 기준으로
**행동 판정**을 내린 뒤, 무관 판정을 제외한 항목만 Hugo 포스트로 커밋하여 GitHub Pages에 배포한다.

**사이트:** https://jeonck.github.io/cti-info/

## 판정 체계

| 판정 | 의미 | 예 |
|---|---|---|
| 🔥 즉시조치 | 활발히 익스플로잇 중인 CVE / 긴급 패치 필요 | CISA KEV 신규 등재, PoC 공개된 Critical CVE |
| 📌 백로그 | 추적할 가치는 있으나 긴급하지 않음 | 신규 CVE 공개(익스플로잇 미확인), 벤더 보안 권고 |
| 📚 학습 | 온톨로지/지식그래프 구조화, TTP·APT 동향, 상호운용성(STIX/TAXII 등) 관련 | 신규 위협 행위자 분석, ATT&CK 매핑 사례, CTI 표준 동향 |
| 무관 | 그 외 전부 → **포스트 생성 안 함** | 마케팅성 제품 홍보, CTI와 무관한 일반 IT 뉴스 |

각 포스트는 `근거`(CVE 심각도/익스플로잇 여부/구조화 관련성)와 `액션`(확인·조사·기록할 구체적 작업 1개)을 담는다.
