---
title: "ChainDrop: npm 1,300개 패키지 감염한 자가 전파형 공급망 공격"
date: 2026-08-04T23:06:05.653692+00:00
verdict: "학습"
tags: ["supply-chain-attack", "npm-malware", "ttp-analysis"]
source: "https://www.bleepingcomputer.com/news/security/massive-chaindrop-npm-supply-chain-attack-infects-hundreds-of-packages/"
source_name: "BleepingComputer"
status: "완료"
---
- **근거:** npm 공급망 공격 캠페인으로, TTP(자가 전파 악성코드·공급망 침해) 및 위협 행위자 동향 분석 관심 분야에 해당
- **액션:** ChainDrop 캠페인의 전파 메커니즘·IOC를 MITRE ATT&CK T1195.001(공급망 소프트웨어 손상)에 매핑하고 STIX Bundle로 초안 작성
